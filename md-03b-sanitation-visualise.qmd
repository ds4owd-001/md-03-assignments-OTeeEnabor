...
3.  How many variables are in the data set?
    ...
    **There are 8 columns - therefore 8 variables.**
...
```{r}
sanitation_2000_2020 <- sanitation |> 
  filter(iso3 == "ZAF") |> 
...
ggplot(sanitation_2000_2020,
  ...
  facet_wrap(~year) +
  labs(
    title = "Percentage Managed Sanitation Services in South Africa (2000 and 2020)",
    x = "Residence Type",
    y = "Percentage"
  )+ 
...
1.  Look at the plot that you created. What do you notice about the order of the bars / order of the legend?
    ...
    **The order is alphabetical based on the categories.**

2.  What would you want to change?

    **The order of the fill could be sorted from largest to smallest percentage. This will allow for a simpler analysis. The graph could also be changed to a percentage graph.**

3.  Why did we remove "safely managed sanitation services" from the data set in Task 3?

    **According to [svn.org](https://www.snv.org/update/what-do-you-mean-when-you-say-its-safely-managed-urban-sanitation-sdg-era), the definition of 'safely managed sanitation services" although simple can be misleading to policymakers to define what safe is for the public and environment perspective.**
...
2.  What do you observe when you look at the code and plot?
    ...
    **The san_lim. san_od, and san_unlim have a simillar distribution.**
